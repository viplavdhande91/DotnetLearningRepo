...
To update the database use this command from the Web project folder (replace `Company.FutureProject` with your project's name):

```powershell
dotnet ef database update -c AppDbContext -p ../Company.FutureProject.Infrastructure/Company.FutureProject.Infrastructure.csproj -s Company.FutureProject.Web.csproj
```
